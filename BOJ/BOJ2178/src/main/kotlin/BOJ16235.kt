...
fun main() {
    ...
    repeat(N){ y ->
        readln().split(" ").forEachIndexed{ x, num ->
...
fun findWay(N: Int): Int {
    ...
    while(queue.size != 0) {
        ...
        if(fishes[n.y][n.x] in 1 until smallFish) {
            while(queue.size != 0) {
                var n2 = queue.poll()
                if(n2.len == n.len){
        ...
        repeat(4) {
            ...
            if(x in 0 until N && y in 0 until N &&
                    fishes[y][x] <= smallFish && !visited[y][x]) {
                visited[y][x] = true
